...
class Society:
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        superior = citizen.get_superior()  # get the superior
        # swap their job
        citizen.job, superior.job = superior.job, citizen.job

        superior.remove_subordinate(citizen.cid)

        c_subordinates = citizen.get_direct_subordinates()
        s_subordinates = superior.get_direct_subordinates()

        s_superior = superior.get_superior()

        citizen._subordinates, superior._subordinates = \
            s_subordinates, c_subordinates
        #
        # for i in citizen.get_direct_subordinates():
        #     citizen.remove_subordinate(i.cid)
        # for j in superior.get_direct_subordinates():
        #     superior.remove_subordinate(j.cid)

        if isinstance(superior, DistrictLeader):
            superior_district = superior.get_district_name()

            new_superior = self.change_citizen_type(superior.cid)
            s = Society(citizen)
            new_citizen = s.change_citizen_type(citizen.cid, superior_district)

            if s_superior:
                s_superior.remove_subordinate(new_superior.cid)
                new_citizen.become_subordinate_to(s_superior)

            # new_citizen._subordinates, new_superior._subordinates = \
            #     s_subordinates, c_subordinates

            new_superior.become_subordinate_to(new_citizen)
        ...
        s_superior.remove_subordinate(superior.cid)
        # citizen.remove_subordinate(citizen.cid)
        ...
        citizen.become_subordinate_to(s_superior)
        superior.become_subordinate_to(citizen)

        # for x in c_subordinates:
        #     x.become_subordinate_to(superior)
        # for y in s_subordinates:
        #     y.become_subordinate_to(citizen)
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        """
        citizen = self.get_citizen(cid)
        if citizen.get_superior():
            while (not isinstance(citizen, DistrictLeader)) and \
                    (citizen.get_superior().rating < citizen.rating):
                citizen = self._swap_up(citizen)
                if not citizen.get_superior():
                    return
